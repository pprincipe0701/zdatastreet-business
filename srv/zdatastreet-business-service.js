...
const { MovimientoDiarioCustom, Establecimientos, MovimientosDiarios } = cds.entities;
...
async function saveMovDiario(req) {
    ...
    try {
        ...
        //Verificar si para el registro del mismo establecimiento ya paso como minimo media hora con respecto al ultimo registro
        let movDiarioLast = await SELECT.one.from(MovimientosDiarios).where({ establecimiento_ID: idEstablecimiento,masterAfluencia_ID:data.idAfluencia,masterTipoPublico_ID:data.idTipoPublico,
             masterEstacionTemporada_ID:data.idEstacionTemporada, registrador:data.registrador, fechaRegistro:fechaRegistro }).orderBy({ref:['createdAt'],sort:'desc'});
        ...
        if(movDiarioLast != null) {
            ...
            if(diffMinutos < 20) {
                ...
                //throw { "Error": "No puede registrar movimiento de el mismo establecimiento antes de 20 minutos despues de registrado el ultimo movimiento" };
                throw "No puede registrar movimiento del mismo establecimiento hasta despues de 20 minutos";
            }
